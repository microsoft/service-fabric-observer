    ...
    public class AzureStorageObserver : ObserverBase
    {
        ...
        private async Task ProcessFilesAsync(string folderPath, CancellationToken token)
        {
            ...
            for (int i = 0; i < files.Length; ++i)
            {
                ...
                bool success = await UploadBlobAsync(file, BlobContainerName, token);
        ...
        private bool Initialize()
        {
            ...
            {
                ObserverLogger.LogWarning("Initialize(): No connection string provided. Aborting..");
                return false;
            }
            ...
            if (isEncrypted)
            {
                ...
                if (StorageConnectionString == null || StorageConnectionString.Length == 0)
                {
                    ObserverLogger.LogWarning("Unable to decrypt ConnectionString.");
                    return false;
                }
            }
            else
        ...
        private async Task<bool> UploadBlobAsync(string filePath, string containerName, CancellationToken token)
        {
            ...
            BlobContainerClient container = new BlobContainerClient(s, containerName);
                ...
                .ContinueWith((response) =>
                {
                    ...
                    else if (response.IsCompletedSuccessfully)
                    {
                        ObserverLogger.LogInfo($"Successfully uploaded file {Path.GetFileName(filePath)} " +
                                               $"to blob container {containerName}.");
                        success = true;
                    }
