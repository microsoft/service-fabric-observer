...
| **warningRGMemoryLimitPercent** | Windows-only. Percentage of Resource Governance 'MemoryInMB/Limit' value currently in use that will trigger a Warning for a service code package. This is how you override the default 90% threshold that AppObserver uses. You can override this value for specific or all services like any other AppObserver threshold. Note: if you supply a decimal value (e.g., 0.70) it is converted to a double (so, it is taken to mean 70.0, so 70%). |  
