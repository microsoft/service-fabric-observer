## FabricObserver 3.3.3 (NET 8, SF Runtime version 10.0 and higher)
...
> FabricObserver 3.3.3 targets SF runtime versions 10.0 and higher. Starting with version 3.3.0, you must deploy the self-contained release package unless you are deploying to a cluster running SF Version 10.1 CU3 or higher, then you can deploy framework-dependent release.
...
**Note: You must deploy this version (3.3.3) to clusters that are running SF 9.0 and above. This version also requires .NET 8.**
...
Copy-ServiceFabricApplicationPackage -ApplicationPackagePath $path -CompressPackage -ApplicationPackagePathInImageStore FO333 -TimeoutSec 1800

#Register FO ApplicationType:

Register-ServiceFabricApplicationType -ApplicationPathInImageStore FO333

#Create FO application (if not already deployed at lesser version):

New-ServiceFabricApplication -ApplicationName fabric:/FabricObserver -ApplicationTypeName FabricObserverType -ApplicationTypeVersion 3.3.3   
...
Start-ServiceFabricApplicationUpgrade -ApplicationName fabric:/FabricObserver -ApplicationTypeVersion 3.3.3 -Monitored -FailureAction rollback
```  
...
**NOTE:** ```SFConfigurationObserver```, which has been deprecated for several releases has been completely removed in 3.3.3. Further, all related settings have been removed from Settings.xml and ApplicationManifest.xml.
