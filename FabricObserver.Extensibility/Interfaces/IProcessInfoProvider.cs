...
namespace FabricObserver.Observers.Utilities
{
    public interface IProcessInfoProvider
    {
        ...
        float GetProcessWorkingSetMb(uint processId, string procName, CancellationToken token, bool getPrivateWorkingSet = false);
        ...
        /// <param name="processId">The id of the process.</param>
        /// <returns></returns>
        float GetProcessPrivateBytesMb(uint processId);
        ...
        float GetProcessAllocatedHandles(uint processId, string configPath = null);
        ...
        /// <returns></returns>
        List<(string ProcName, uint Pid)> GetChildProcessInfo(uint parentPid, NativeMethods.SafeObjectHandle handleToSnapshot);
        ...
        double GetProcessKvsLvidsUsagePercentage(string procName, CancellationToken token, uint procId = 0);
    }
}